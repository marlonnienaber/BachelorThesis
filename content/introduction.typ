...
The technological change over the 20th century up until the beginning of the 21th century has impacted the educational paradigm of many universities. The common approach has shifted from face-to-face classroom learning - that is by nature synchronous and location-bound - to a model blending the traditional format with asynchronous and location-independent online learning #cite(<And2020>) #cite(<Des2008>) /*#cite(<Halv2009>)*/#cite(<Hara2000>). With this change come advantages, but also some challenges.

The invention of the internet #cite(<Mow2002>) in the end of the 1960s #cite(<Mow2002>) and the rise of personal computers during the end of the 1970s #cite(<Abba1999>) increased public access to digital resources and paved the way for universities to integrate online materials into their curricula. The development of the first learning management systems during the 1990s #cite(<Aldh2024>), which are web applications that are specifically designed to support teaching, is symptomatic of this trend.
...
This blended model offers room for flexibility when it comes to individual student needs by allowing learners to partially decide when and where to engage with course content. However, despite this benefit, there are also some challenges associated with Artemis’s approach.
...
The increased complexity of the blended course environment amplifies the need for effective planning and personal time management. In a classical teaching approach, learning is centered around lectures. With Artemis however, courses combine lectures with several exercise formats and tutorial group sessions. Each of these course-related concepts is potentially associated to multiple deadlines and dates. 

Whether an event is relevant to a user can depend on his role in the given course. For instance, the start and end date of a lecture are relevant for both students and instructors, while the start and end date of tutorials are only important to students and tutors of the respective tutorial groups. 
...
Currently, Artemis does not offer an effective, user-friendly tool that clearly communicates important dates in a centralized manner. Users must log into Artemis and manually navigate through various course components to find relevant events, making personal time management unnecessarily difficult.

== Motivation
To address this issue, integrating a digital calendar into Artemis would provide significant benefits. This calendar would allow students, instructors, editors, and tutors to easily view all important events. By implementing a subscription mechanism, users could synchronize the Artemis calendar with their personal device's calendars, eliminating the need to log into Artemis and further streamlining the time management process.
...
This thesis aims to extend Artemis with a time management feature. The center piece of this feature is a calendar that displays all important events related to a course. The calendar supports a variety of event types and provides a mechanism to subscribe external calendar applications through iCalendar feeds. Based on this, we define the following objectives:

==== Support Derived Events
In a first step we want to enable the calendar to display so-called _Derived Events_. The idea here is to include important dates of objets that are already part of Artemis in the calendar. Examples for such objects are lectures, exercises, tutorial group sessions and exams. Each of these objects can have multiple date properties from which the system can derive events and include them in the calendar.

==== Enable Lecture Scaffolding and Exercise Planning
Additionally, we address a problem with events derived from lectures and exercises. These events appear only after the course team creates the corresponding lecture or exercise. Since this setup requires effort, the course team often adds them just a few days before the lecture or exercise takes place. We plan to add functionality that lets students see lecture and exercise events earlier. To achieve this, we want to extend Artemis to allow quick scaffolding of lectures without specifying their content and the creation of planned exercises that serve as proxies for the real exercises.
...
@architecture describes how we transfer application domain concepts from @requirements into the solution domain. This includes specifying design goals, defining the subsystem decomposition and designing objects that we use in the implementation.
